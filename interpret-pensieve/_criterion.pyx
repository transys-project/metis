...
cdef class Gini(ClassificationCriterion):
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        # printf('----- children_impurity -----\n')
        # for k in range(self.n_outputs):
        #     for c in range(n_classes[k]):
        #         printf('i = %d sum_left[i] = %.0f sum_right[i] = %.0f sum_total[i] = %.0f\n', c, sum_left[c], sum_right[c], sum_total[c])
        ...
        for k in range(self.n_outputs):
            ...
            #
            for c in range(n_classes[k]):
                count_parent_k = sum_total[c]
                if count_parent_k < 1:
                    continue
                count_k = sum_left[c]
                q = count_k/count_parent_k
                sq_count_left += q * q
                q_sum_left += q
            ...
            gini_left += 1.0 - sq_count_left / (q_sum_left * q_sum_left)
            gini_right += 1.0 - sq_count_right / (q_sum_right * q_sum_right)
        ...
        # printf('gini_left %.8f gini_right %.8f\n', impurity_left[0], impurity_right[0])
