...
def _sample(obss, acts, qs, copies, max_pts, is_reweight, over_two, over_four):
    # Step 1: Compute probabilities
    ps = np.max(qs, axis=1) - np.min(qs, axis=1)
    ps = (np.array(ps) * (np.array([i >= 0 for i in acts]) + np.array([i == 2 for i in acts]) * (over_two - 1) +
                         np.array([i == 4 for i in acts]) * (over_four - 1))).tolist()
    ps = ps / np.sum(ps)
...
def train_dagger(env, teacher, student, max_iters, n_batch_rollouts, max_samples, train_frac,
                 is_reweight, n_test_rollouts, parameters, feature_names, over_two, over_four):
    # Step 0: Setup
    ...
    for i in range(max_iters):
        ...
        cur_serialized_obss, cur_acts, cur_qs, cur_copies = _sample(np.array(serialized_obss), np.array(acts), np.array(qs), np.array(copies),
                                                        max_samples, is_reweight, over_two, over_four)
