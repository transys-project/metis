...
from ..pensieve_test.a3c_python3 import *
from .log import *
...
def get_rollout(env, policy, parameters, is_student=True):
    ...
    while True:
        ...
        if end_of_video:
            ...
            break
        rollout.append((state, bit_rate, reward, serilized_state))
    return rollout
...
def train_dagger(env, teacher, student, max_iters, n_batch_rollouts, max_samples, train_frac,
                 is_reweight, n_test_rollouts, parameters):
    # Step 0: Setup
    ...
    obss.extend((obs for obs, _, _, _ in trace))
    acts.extend((act for _, act, _, _ in trace))
    serialized_obss.extend((serialized_obs for _, _, _, serialized_obs in trace))
    qs.extend(teacher.predict_q(np.array([obs for obs, _, _, _ in trace])))
    ...
    for i in range(max_iters):
        ...
        cur_serialized_obss, cur_acts, cur_qs = _sample(np.array(serialized_obss), np.array(acts), np.array(qs),
                                                        max_samples, is_reweight)
        log('Training student with {} points'.format(len(cur_serialized_obss)), INFO)
        student.train(cur_serialized_obss, cur_acts, train_frac)
        ...
        student_serialized_obss = [serialized_obss for _, _, _, serialized_obss in student_trace]
        ...
        for obs in student_obss:
            action_prob = teacher.predict(np.reshape(obs, (1, parameters['S_INFO'], parameters['S_LEN'])))
            action_cumsum = np.cumsum(action_prob)
...
def train_dagger(env, teacher, student, max_iters, n_batch_rollouts, max_samples, train_frac,
    ...
    return max_student
>>>>>>> 41a7950cd84c5ae92205692ea915a2496a5ea76f
